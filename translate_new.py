...
input_directory_1 = '/Users/danielenricobotta/scripts/translator/pokeyellow/text'
output_directory_1 = '/Users/danielenricobotta/scripts/translator/pokeyellow_turkish/text'

input_directory_2 = '/Users/danielenricobotta/scripts/translator/pokeyellow/data/text'
output_directory_2 = '/Users/danielenricobotta/scripts/translator/pokeyellow_turkish/data/text'

log_filepath = '/Users/danielenricobotta/scripts/translator/pokeyellow_turkish/completed_files.log'
...
placeholders = {literal: f"{{PLACEHOLDER{literal[1:-1]}PLACEHOLDER}}" for literal in literals}
                ...
                if match:
                    ...
                    if directive in ['text']:
                        if len(translated_text) > MAX_LINE_LENGTH  and not re.search(r'<[^>]+>', translated_text):
                            half = int((len(translated_text) / 2))
                            translated_lines.append(f'\ttext "{translated_text[0:half]}"')
                            translated_lines.append(f'\tline "{translated_text[half:-1]}"')
                        else:
                            translated_lines.append(f'\ttext "{translated_text}"')
                    else: 
                        if len(translated_text) > MAX_LINE_LENGTH  and not re.search(r'<[^>]+>', translated_text):
                            half = int((len(translated_text) / 2))
                            translated_lines.append(f'\tpara "{translated_text[0:half]}"')
                            translated_lines.append(f'\tline "{translated_text[half:-1]}"')
